...
class CommentAdapter(private val context: Context, private var ds: List<Comment> = emptyList()): RecyclerView.Adapter<CommentAdapter.CommentHolder>() {

    private lateinit var helper: DatabaseHelper
    private lateinit var binding: ItemBinhLuanBinding
    inner class CommentHolder(private val binding: ItemBinhLuanBinding):RecyclerView.ViewHolder(binding.root){
        private var currentComment: Comment? = null
        fun bind(comment:Comment){
            currentComment = comment
            helper = DatabaseHelper(context)
            val db = helper.readableDatabase
            val query = "select * from users where user_id in (select user_id from comments_book)"
